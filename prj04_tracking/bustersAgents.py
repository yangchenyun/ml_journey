    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        mostProbableGhostPosList = [d.argMax()
                                    for d in livingGhostPositionDistributions]
        closestGhostIndex = min(range(len(mostProbableGhostPosList)),
                                key=lambda i: self.distancer.getDistance(pacmanPosition, mostProbableGhostPosList[i]))
        target = mostProbableGhostPosList[closestGhostIndex]

        def action_score(action):
            newPos = Actions.getSuccessor(pacmanPosition, action)
            return util.manhattanDistance(newPos, target)

        bestAction = min(gameState.getLegalActions(), key=action_score)
