...
def constructBayesNet(gameState: hunters.GameState):
    ...
    cell_domain = list(itertools.product(range(X_RANGE), range(Y_RANGE)))
    distance_domain = list(range(X_RANGE + Y_RANGE + MAX_NOISE - 1))
    ...
    variableDomainsDict = {
        ...
        GHOST0: cell_domain,
        GHOST1: cell_domain,
        OBS0: distance_domain,
        OBS1: distance_domain
    }
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # Eliminate variable in the linear order
        for joinVariable in eliminationOrder:
            ...
            if len(joinedFactor.unconditionedVariables()) > 1:
                currentFactorsList.append(
                    eliminate(joinedFactor, joinVariable))

        joinAfterElimination = joinFactors(currentFactorsList)

        return normalize(joinAfterElimination)
...
class InferenceModule:
    ...
    def getPositionDistributionHelper(self, gameState, pos, index, agent):
        ...
        if ghostPosition in pacmanSuccessorStates:  # Ghost could get caught
            ...
            dist[jail] = mult  # Then it possible to be in jail?
        else:
            mult = 0.0
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        position is known.
        """
        # p(ghost|o) = p(ghost)*p(o|ghost)/p(o)
        pacmanPos = gameState.getPacmanPosition()
        jailPos = self.getJailPosition()
        for ghostPos in self.allPositions:
            post_p = self.getObservationProb(
                observation, pacmanPos, ghostPos, jailPos)
            if post_p:
                self.beliefs[ghostPos] *= post_p
            else:
                self.beliefs[ghostPos] *= 0.0
