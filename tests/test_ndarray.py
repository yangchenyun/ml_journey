...
_DEVICES = [nd.cpu(), 
            ...
            pytest.param(nd.triton(),
                         marks=pytest.mark.skipif(not nd.triton().enabled(), reason="No Triton")),
            ]
...
], ids=["identity", "transpose", "broadcast_to", "reshape1", "reshape2", "reshape3", "getitem1", "getitem2", "transposegetitem", "slice"])
...
@pytest.mark.parametrize("params", [
    {
        "lhs": ShapeAndSlices(1,1)[:,:],
        "rhs": ShapeAndSlices(1,1)[:,:]
    },
    {
        "lhs": ShapeAndSlices(1,1)[0, 0],
        "rhs": ShapeAndSlices(1,1)[0, 0]
    },
    {
        "lhs": ShapeAndSlices(3, 3, 3)[:, :, :],
        "rhs": ShapeAndSlices(3, 3, 3)[:, :, :]
    },
    {
        "lhs": ShapeAndSlices(3, 3, 3)[:, :, :],
        "rhs": ShapeAndSlices(3, 3, 4)[:, :, 1:]
    },
    {
        "lhs": ShapeAndSlices(3, 3, 3)[:, :, :],
        "rhs": ShapeAndSlices(3, 4, 4)[:, :-1, 1:]
    },
    {
        "lhs": ShapeAndSlices(3, 3, 3)[:, 0, 0],
        "rhs": ShapeAndSlices(3, 7, 7)[:, 0, 0]
    },
    {
        "lhs": ShapeAndSlices(3, 3, 3)[:, 0, 0],
        "rhs": ShapeAndSlices(4, 7, 7)[1:, 0, 0]
    },
    {
        "lhs": ShapeAndSlices(4, 5, 6)[1:, 0, 0],
        "rhs": ShapeAndSlices(4, 7, 7)[1:, 0, 0]
    },
    {
...
def test_scalar_power(device):
    ...
    B = nd.array(A, device=device)
    for r in [1., 5., .5]:
        np.testing.assert_allclose(np.power(A, r), (B**r).numpy(), atol=1e-5, rtol=1e-5)
