...
def equal(A, B):
    ...
    return domain_equal and all(A[k] == B[k] for k in sparse_keys(A, B))


def sparse_keys(A, B):
    return set(A.f.keys()).union(set(B.f.keys()))
...
def add(A, B):
    ...
    """
    assert A.D == B.D
    return Mat(A.D, {k: A[k] + B[k] for k in sparse_keys(A, B)})
...
def matrix_matrix_mul(A, B):
    ...
    """
    assert A.D[1] == B.D[0]
    a_rows = {row: Vec(A.D[1], {col: A[row, col]
                       for col in A.D[1]}) for row in A.D[0]}
    b_cols = {col: Vec(B.D[0], {row: B[row, col]
                       for row in B.D[0]}) for col in B.D[1]}
    return Mat((A.D[0], B.D[1]), {(ra, cb): a_rows[ra] * b_cols[cb]
        for ra in a_rows for cb in b_cols})
    ...
    def __str__(M, rows=None, cols=None):
        ...
        def format_col(c):
            if isinstance(M[r, c], int) or isinstance(M[r, c], float):
                return '{0:>{1}.{2}G}'.format(M[r, c], colw[c], numdec)
            else:
                print(M[r, c], colw[c])
                return '{0:>{1}}'.format(M[r, c], colw[c])
